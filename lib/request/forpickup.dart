...
class ForPickupPage extends StatefulWidget {
  ...
  @override
  State<ForPickupPage> createState() => _ForPickupPageState();
}
...
class _ForPickupPageState extends State<ForPickupPage> {
      ...
      PlutoColumn(
          ...
          renderer: (rendererContext) {
            return Row(
                ...
                children: [
                  kApprovedButton(
                    onTap: () => handleCompleted(rendererContext.row),
                  ),
                ]);
          }),
    ]);
  ...
  Future<void> handleCompleted(PlutoRow row) async {
    try {
      var result = await Dialogs.confirmationDialog(
          context,
          "Mark this request as completed (${row.cells['description']?.value})?",
          "");
